...
import type { VerifyEmailRequestDto, VerifyEmailResponseDto } from '../registration/registration.dto.js';
...
export class EmailVerificationService {
  ...
  async verifyEmail(
    ...
    if (user.emailVerified) {
      log.info({ userId: user.id, email: normalizedEmail }, 'Email verification attempt for already verified email');
    ...
    if (isLocked) {
      log.error({ userId: user.id, email: normalizedEmail }, 'Email verification blocked - too many attempts');
    ...
    if (token.expiresAt < now) {
      ...
      log.debug({ userId: user.id, attempts: newAttemptCount }, 'Incremented OTP attempts for expired token');
    ...
    if (token.tokenHash !== otpHash) {
      ...
      if (newAttemptCount >= 5) {
        log.error({ userId: user.id, attempts: newAttemptCount }, 'User locked out after 5 failed attempts');
